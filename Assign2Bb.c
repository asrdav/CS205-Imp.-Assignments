//To implement search in sorted array by splitting the input into two sets of size two-fifth and three-fifth.
#include<stdio.h>

int num1[100005];		//num1 function made to store the numbers.
...
int partition(int low,int high)			//partition function made to partition the array for quick sort.
{
	...
	while(1)		//while loop made to create the partition.
	{
...
void quick_sort(int start,int last)		//quick_sort function called to sort the array.
{
        if(start<last)
        {
        int p=partition(start,last);
    	quick_sort(start,p-1);			//quick_sort function recursively called.
        quick_sort(p+1,last);			//quick_sort fucntion recursively called.
        }
}
...
int search(int begin,int end)			//search function made to search the given no.
{
	...
	{
	int mid1=begin+2*(end-begin)/5;		//mid1 is the end of the first interval.
	if(find_num==num1[mid1])		//if block made to terminate the condition.
	return mid1;		
	...
	return search(mid1+1,end);		//search function recursively called.
